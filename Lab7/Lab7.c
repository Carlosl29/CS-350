...
const char *aFinalOPCodes[] = {"GETC", "OUT", "PUTS", "DMP", "MEM", "Null", "Null", "Null", "Null", "Null"};
	...
	{		
		...
		{	
			...
			if(cmd_char == 'q')
			{
				...
				execute_command('q', pCpu);
				done = 1;
			}
			else if((cmd_char == 'h') || (cmd_char == '?'))
			{
				printf("Character 'h or ?' entered.");
				execute_command('h', pCpu);
			}
			else if(cmd_char == 'd')
			{
				printf("Character 'd' entered.");
				printf("Dumping info...");
				execute_command('d', pCpu);				
			}
			else if(cmd_char == '\n')
			{
				printf("Nothing entered, default run of cycle is 1...");
				one_instruction_cycle(pCpu);
			}
			else
			{
				nbr_cycles = (int)cmd_char;
				if(nbr_cycles < 1){
					printf("Incorrect input");
				}
				else
				{
					many_instruction_cycles(nbr_cycles, pCpu);
				}
			}
		}
	}
...
void help_message(void) 
{
	...
	printf("An integer > 0 to execute that many instructions cycles\n");
...
void many_instruction_cycles(int nbr_cycles, CPU *pCpu) {
	...
	if(nbr_cycles <= 0)
	{
		printf("Incorrect num of cycles to do");
	}
	else if(nbr_cycles > MAX_NBR_CYCLES)
	{
		nbr_cycles = MAX_NBR_CYCLES;
	}
...
void one_instruction_cycle(CPU *pCpu) {
	...
	else if(pCpu->running == 1)
	{
		// Get instruction and increment pc
		//
		int instr_loc = pCpu->pc;  // Instruction's location (pc before increment)
		pCpu->ir = pCpu->mem[pCpu->pc];
		(pCpu->pc)++;

		// Decode instruction into opcode, reg_R, addr_MM, and instruction sign
		//
		//the instruction is in here
		
		pCpu -> opcode = ((pCpu->ir) >> 3 );
		pCpu -> reg_R = (((pCpu->ir) << 1) >> 3);
		pCpu -> addr_MM = (((pCpu->ir) << 2) >> 2);
		...
		// *** STUB ****
		printf("opcode = %d\treg_R = %d\taddr_MM = %d\tinstr_sign = %d",pCpu -> opcode,pCpu -> reg_R, pCpu -> addr_MM, pCpu->instr_sign);
		// Echo instruction
		// *** STUB ***
	}
	// *** STUB ****
